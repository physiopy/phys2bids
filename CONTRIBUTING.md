...
Remember to [clone your fork][link_clonerepo] of `phy2sbids` to your local machine, which will allow you to make local changes to `phys2bids`.
...
<!-- For changes to the codebase, we suggest using our development Docker container which will run all the necessary checks and tests to ensure your code is ready to be merged into `phys2bids`!
...
from within your local `phys2bids` repository.
(**N.B.** It is possible that, depending on your Docker setup, you may need to increase the amount of memory available to Docker in order to run the `phys2bids` test suite.
...
from the `docs` directory in your local `phys2bids` repository. You should then be able to access the rendered files in the `docs/_build` directory, and view them in your browser. -->
...
The documentation for `phys2bids` is written using [ReStructuredText][restructuredtext].
...
to provide an easily accessible space for accessing `phys2bids` documentation.
