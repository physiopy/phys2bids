...
acq2txt "${chsel}" -o rm.${sub}transform.tsv ${in}

# Remove first line
csvtool -t TAB -u TAB drop 1 rm.${sub}transform.tsv > rm.${sub}drop.tsv

# Get time and trigger in separate files (correcting trigger offset)
echo "Separating time and trigger (channel ${chtrig})"
csvtool -t TAB col 1 rm.${sub}drop.tsv > rm.${sub}time.1D
# csvtool -t TAB col 2 rm.${sub}trigger.tsv > rm.${sub}trigger.1D
let chtrig+=2
csvtool -t TAB col ${chtrig} rm.${sub}drop.tsv > rm.${sub}trigger.1D

# Derive trigger to check number of tp in file, then threshold and count number of tp.
echo "Counting trigger points"
1d_tool.py -infile rm.${sub}trigger.1D -derivative -write rm.${sub}trigger_deriv.1D -overwrite
1deval -a rm.${sub}trigger_deriv.1D -b=${thr} -expr 'ispositive(a-b)' > rm.${sub}trigger_thr.1D

ntpf=$( awk '{s+=$1} END {printf "%.0f", s}' rm.${sub}trigger_thr.1D )

# Find time of first timepoint above 0.5: the first trigger
echo "Extracting other info for json file"
evawk="awk '\$${chtrig}>${thr}{print; exit}' rm.${sub}drop.tsv"
tza=( $( eval "${evawk}" ) )
