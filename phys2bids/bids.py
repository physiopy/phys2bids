...
def bidsify_units(orig_unit):
    ...
    LGR.warning(
        f"The given unit {orig_unit} does not have aliases, " f"passing it as is"
    )
    return orig_unit
...
def use_heuristic(heur_file, sub, ses, filename, outdir, run="", record_label=""):
    ...
    if not bids_keys["task"]:
        LGR.warning(
            f"The heuristic {heur_file} could not deal with" f"{Path(filename).stem}"
        )
...
def dataset_description_file(outdir):
    ...
    if not os.path.exists(file_path):
        LGR.warning(
            "phys2bids could not find dataset_description.json,"
