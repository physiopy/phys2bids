...
class BlueprintInput:
        ...
        for n, channel in enumerate(self.timeseries):
            ...
            for i in ["start", "stop", "step"]:
                if idx_dict[i]:
                    idx_dict[i] = int(
                        np.floor(
                            self.freq[n] / self.freq[self.trigger_idx] * idx_dict[i]
                        )
                    )
    ...
    def delete_at_index(self, idx):
        ...
        if self.trigger_idx == idx:
            LGR.warning(
                "Removing trigger channel - are you sure you are doing"
                "the right thing?"
            )
            self.trigger_idx = 0
    ...
    def check_trigger_amount(self, thr=None, num_timepoints_expected=0, tr=0):
        ...
        if num_timepoints_expected:
            ...
            if num_timepoints_found > num_timepoints_expected:
                ...
                LGR.warning(
                    f"Found {timepoints_missing} timepoints" " less than expected!"
                )
                if tr:
                    LGR.warning(
                    ...
                    LGR.warning(
                        "Can't correct time offset - you should " "specify the TR"
                    )
            ...
            LGR.warning(
                "The necessary options to find the amount of timepoints "
                "were not provided."
            )
        self.thr = thr
        self.time_offset = time_offset
    ...
    def print_info(self, filename):
        ...
        for ch in range(1, self.ch_amount):
            info = info + (
                f"{ch:02d}. {self.ch_name[ch]};" f" sampled at {self.freq[ch]} Hz\n"
            )
        info = info + "------------------------------------------------\n"
...
class BlueprintOutput:
    ...
    def save(self):
        ...
        save_json(self.filename + ".json", self.freq, self.start_time, self.ch_name)
