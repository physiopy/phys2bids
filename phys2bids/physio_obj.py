...
class BlueprintInput():
    ...
    def check_trigger_amount_std(self, chtrig=1, num_timepoints_expected=0, tr=0):
        from itertools import groupby
        """
        finds proper threshold according to twice the std of the trigger channel
        ...
        self.timeseries:
            ...
            the time of first trigger.
        """
        LGR.info('Counting trigger points according to std method')
        # Use twice the std of the trigger channel to get the threshold
        # comparing it to a given threshold.
        trigger = self.timeseries[chtrig]
        thr_std = np.mean(trigger) + 2 * np.std(trigger)
        timepoints = trigger > thr_std
        num_timepoints_found = len([is_true for is_true, _ in groupby(timepoints,
                                    lambda x: x != 0) if is_true])
        LGR.info(f'number of expected timepoints according to std method'
                 f'is {num_timepoints_found} the threshold is {thr_std}')
        time_offset = self.timeseries[0][timepoints.argmax()]
        if num_timepoints_expected:
            LGR.info('Checking number of timepoints')
            if num_timepoints_found > num_timepoints_expected:
