...
def test_integration_heuristic(skip_integration, multifreq_lab_file):
    ...
    assert check_string(
        logger_info, "phys2bids version", current_version["version"], is_num=False
    )
    ...
    for suffix in [".json", ".tsv.gz"]:
        for freq in ["40", "100", "500", "1000"]:
            assert isfile(
                join(test_path_output, f"{base_filename}{freq}Hz_physio{suffix}")
            )

    # ##### Checks for 40 Hz files
    # Read log file (note that this file is not the logger file)
    log_filename = (
        "sub-006_ses-01_task-test_rec-biopac_run-01_recording-40Hz_physio.log"
    )
    ...
    # Checks json file
    json_filename = (
        "sub-006_ses-01_task-test_rec-biopac_run-01_recording-40Hz_physio.json"
    )
    ...
    # Read log file (note that this file is not the logger file)
    log_filename = (
        "sub-006_ses-01_task-test_rec-biopac_run-01_recording-100Hz_physio.log"
    )
    with open(join(conversion_path, log_filename)) as log_info:
        log_info = log_info.readlines()
    ...
    # Checks json file
    json_filename = (
        "sub-006_ses-01_task-test_rec-biopac_run-01_recording-100Hz_physio.json"
    )
    with open(join(test_path_output, json_filename)) as json_file:
        json_data = json.load(json_file)
